...
STOCK_DEVICE="$3"
KERNEL_PKG_PATH="${4:-}"
OUTPUT_ZIP_NAME="${5:-ProjectDozG_${TARGET_DEVICE}_Port_For_${STOCK_DEVICE}_Recovery.zip}"
...
OTA_CERT="$QT_DIR/security/ota_keys/ota_key.x509.pem"
OTA_KEY="$QT_DIR/security/ota_keys/ota_key.pk8"
SIGNAPK_BIN="$QT_DIR/bin/signapk/signapk"

# If public cert does not exist, extract it from otacerts.zip
if [ ! -f "$OTA_CERT" ] && [ -f "$QT_DIR/OTA/system/etc/security/otacerts.zip" ]; then
    mkdir -p "$QT_DIR/ota_keys"
    unzip -p "$QT_DIR/OTA/system/etc/security/otacerts.zip" ota_key.x509.pem > "$OTA_CERT" 2>/dev/null || true
fi

# If private key was passed via environment variable (e.g. GitHub Actions secret)
if [ ! -f "$OTA_KEY" ] && [ -n "${OTA_KEY_BASE64:-}" ]; then
    mkdir -p "$QT_DIR/ota_keys"
    echo "$OTA_KEY_BASE64" | base64 -d > "$QT_DIR/security/ota_keys/ota_key.pk8"
    chmod 600 "$QT_DIR/security/ota_keys/ota_key.pk8"
    OTA_KEY="$QT_DIR/security/ota_keys/ota_key.pk8"
fi
